...
export async function analyzeWebsite(url, puppeteerInstance) {
  ...
  let metadata = {};
  try { metadata = await scraper({ html, url: normalizedUrl }); } catch {}
  ...
  try {
    const browser = await puppeteerInstance.launch({ headless: true, args: ["--no-sandbox", "--disable-setuid-sandbox"] });
  ...
  try {
    const chrome = await launch({ chromePath: puppeteer.executablePath(), chromeFlags: ["--headless"] });
    const result = await lighthouse(normalizedUrl, { logLevel: "error", output: "json", onlyCategories: ["performance"], port: chrome.port });
